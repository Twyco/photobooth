...
<template>
  <div class="relative flex items-center justify-center h-screen w-screen overflow-hidden">
    <div class="relative w-full h-full lg:max-w-[75vw] lg:max-h-[75vh] lg:w-fit">
      <Link
        v-if="prevImg"
        :href="prevImg"
        class="absolute z-10 left-1 top-1/2 transform opacity-80 -translate-y-1/2 rounded-full p-2
              lg:-left-12 lg:bg-primary lg:bg-opacity-50 lg:hover:bg-opacity-70 lg:px-3 lg:py-5"
        replace
      >
        <ArrowLeft />
      </Link>
      ...
      <ProgressiveImage
        ...
        @loaded-compressed-img="progressiveImg && progressiveImg.startLoadOriginalImg()"
      />

      <Link
        v-if="nextImg"
        :href="nextImg"
        class="absolute z-10 right-1 top-1/2 transform -translate-y-1/2 rounded-full p-2
              lg:-right-12 lg:bg-primary lg:bg-opacity-50 lg:hover:bg-opacity-70 lg:px-3 lg:py-5"
        replace
      >
        <ArrowRight />
      </Link>
